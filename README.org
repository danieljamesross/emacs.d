  ...
  (defun display-all-vterms ()
    ...
    (let (buffers-in-window (buffer-in-window-list))
      (dolist (buffer (buffer-list))
        (when (and (string-match "\\*vterminal" (buffer-name buffer))
                   (not (memq buffer buffers-in-window)))
          (set-window-buffer (split-window-right (get-largest-window)) buffer)))
      (balance-windows)))
#+end_src
*** Node
    ...
    #+begin_src emacs-lisp
      ...
      (defun set-prettier-args ()
        ...
        (let* ((node-path "node_modules/.bin")
          ...
          (if (file-exists-p (first prettier-config))
