...
#+begin_src emacs-lisp
  (use-package vterm
    ...
    (defun djr/vterm-disable-display-modes ()
      "Disable line-numbers, prettify-symbols, and ligature modes in vterm.
  vterm assumes a fixed-cell grid, so glyph substitution and gutter
  columns desync cursor positioning and produce ghost text on redraw."
