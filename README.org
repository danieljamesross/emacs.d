      ...
      (defun check-file-exists-or-clone (file repo)
        (let ((expanded-filname (expand-file-name file)))
          (if (file-exists-p expanded-filname)
            ...
            (async-shell-command (format "git clone -q %s %s" repo expanded-filname))
            (princ (format "Cloning repo %s into %s" repo expanded-filname)))))
    #+end_src
        ...
        (if (find-font (font-spec :name font-name))
          ...
          (let* ((temp-folder (expand-file-name "/private/tmp/hack"))
                 ...
                 (shell-command (format "git clone -q %s %s" font-repo temp-folder)
                                           output-buffer)
                 (shell-command (format "mv %s/font/* %s" temp-folder font-folder))
                 (set-default-font font-name))))
    #+end_src
** PATH
   #+BEGIN_SRC emacs-lisp
...
*** eslint-fix
    #+begin_src emacs-lisp
      (unless (file-exists-p (expand-file-name "~/.emacs.d/eslint-fix/"))
        (shell-command "git clone https://github.com/codesuki/eslint-fix.git eslint-fix"))
      (unless (file-exists-p (expand-file-name "/usr/local/bin/eslint"))
        (shell-command "npm i -g eslint"))
      (use-package eslint-fix
        :ensure t
        :load-path "~/.emacs.d/eslint-fix/"
